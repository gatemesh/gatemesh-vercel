...
import { MeshRole, NodeType } from '@/types/agriculture';
...
export function PowerStatusCard({ nodeId }: Props) {
  ...
  // Convert string ID to numeric for history store (temp workaround)
  const numericNodeId = parseInt(nodeId) || 0;
  const batteryHistory = useDataHistoryStore(
    (state) => state.getBatteryHistory(numericNodeId, 20)
  );
  const signalHistory = useDataHistoryStore(
    (state) => state.getSignalHistory(numericNodeId, 20)
  );
  ...
  useEffect(() => {
    if (node) {
      addBatteryLevel(numericNodeId, node.battery);
      addSignalStrength(numericNodeId, node.rssi);
    }
  }, [node, numericNodeId, addBatteryLevel, addSignalStrength]);

  if (!node) return null;

  const isCharging = (node.solar || 0) > node.battery;
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-4 text-sm text-gray-600">
          <div className="flex items-center gap-1">
            ...
            <span>{node.nodeTypes.map(t => NodeType[t]).join(', ')}</span>
          </div>
          <div className="flex items-center gap-1">
            <Radio className="w-4 h-4" />
            <span>{MeshRole[node.meshRole]}</span>
          </div>
        </div>
      </div>
